...
CoD.PrivateGameLobby.DoesGametypeSupportBots = function (Gametype)
	...
	for Index = 1, #ValidGametypes, 1 do
		if Gametype == ValidGametypes[Index] then
			return true
		end
	end
	return false
...
CoD.PrivateGameLobby.PopulateButtons_Project_Zombie = function (PrivateGameLobbyButtonPane, IsHost)
	if IsHost == true then
		...
		for Key1, GameOptions in pairs(CoD.Zombie.GameOptions) do
			GametypeIsValid = false
			if GameOptions.gameTypes ~= nil then
				for Key2, GameOptionGametype in pairs(GameOptions.gameTypes) do
					if GameOptionGametype == Gametype then
			...
			if GametypeIsValid and GameOptions.maps ~= nil then
				for Key3, GameOptionMapname in pairs(GameOptions.maps) do
					if GameOptionMapname == Mapname then
						MapIsValid = true
						break
					end
				end
			...
			if GametypeIsValid then
				local GametypeSettingButtonList = PrivateGameLobbyButtonPane.body.buttonList:addGametypeSettingLeftRightSelector(PrivateGameLobbyButtonPane.panelManager.m_ownerController, Engine.Localize(GameOptions.name), GameOptions.id, Engine.Localize(GameOptions.hintText), f13_local1)
				for Index = 1, #GameOptions.labels, 1 do
					GametypeSettingButtonList:addChoice(PrivateGameLobbyButtonPane.panelManager.m_ownerController, Engine.Localize(GameOptions.labels[Index]), GameOptions.values[Index])
				end
				GametypeSettingButtonList:registerEventHandler("gain_focus", CoD.PrivateGameLobby.ButtonGainFocusZombie)
				GametypeSettingButtonList:registerEventHandler("lose_focus", CoD.PrivateGameLobby.ButtonLoseFocusZombie)
				GametypeSettingButtonList:registerEventHandler("start_game", GametypeSettingButtonList.disable)
				GametypeSettingButtonList:registerEventHandler("cancel_start_game", GametypeSettingButtonList.enable)
				GametypeSettingButtonList:registerEventHandler("gamelobby_update", CoD.PrivateGameLobby.ButtonGameLobbyUpdate_Zombie)
			end
		end
		...
		if true == CoD.useController and not PrivateGameLobbyButtonPane:restoreState() then
...
CoD.PrivateGameLobby.ButtonGameLobbyUpdate_Zombie = function (GametypeSettingButtonList, f14_arg1)
	GametypeSettingButtonList:refreshChoice()
	GametypeSettingButtonList:dispatchEventToChildren(f14_arg1)
end

CoD.PrivateGameLobby.ButtonGainFocusZombie = function (GametypeSettingButtonList, ClientInstance)
	CoD.CoD9Button.GainFocus(GametypeSettingButtonList, ClientInstance)
	GametypeSettingButtonList:dispatchEventToParent({
...
CoD.PrivateGameLobby.ButtonLoseFocusZombie = function (GametypeSettingButtonList, ClientInstance)
	CoD.CoD9Button.LoseFocus(GametypeSettingButtonList, ClientInstance)
	GametypeSettingButtonList:dispatchEventToParent({
		name = "enable_sliding_zm",
		enableSliding = true,
		controller = ClientInstance.controller
...
CoD.PrivateGameLobby.LeaveLobby_Project_Multiplayer = function (PrivateGameLobbyWidget, ClientInstance)
	...
	if Engine.SessionModeIsMode(CoD.SESSIONMODE_OFFLINE) == true or Engine.SessionModeIsMode(CoD.SESSIONMODE_SYSTEMLINK) == true then
		...
		CoD.resetGameModes()
		if CoD.isPS3 then
			Engine.ExecNow(ClientInstance.controller, "signoutSubUsers")
		end
...
CoD.PrivateGameLobby.LeaveLobby_Project_Zombie = function (PrivateGameLobbyWidget, ClientInstance)
	...
	if not CoD.isPC then
		PrivateGameLobbyWidget:registerEventHandler("confirm_leave_animfinished", CoD.PrivateGameLobby.LeaveLobby_Project_Zombie_After_Animation)
	end
