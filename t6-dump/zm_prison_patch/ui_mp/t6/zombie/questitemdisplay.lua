...
CoD.QuestItemDisplay.AddPersistentIcon = function (f2_arg0)
	...
	Widget:setLeftRight(true, false, f2_local0, f2_local0 + CoD.QuestItemDisplay.ContainerSize)
	Widget:setTopBottom(true, false, f2_local1, f2_local1 + CoD.QuestItemDisplay.ContainerSize)
	Widget:setAlpha(0)
	f2_arg0.persQuestContainer = Widget
	if not CoD.Zombie.LocalSplitscreenMultiplePlayers then
		local Widget = LUI.UIText.new()
		Widget:setLeftRight(true, false, 0, 250)
		Widget:setTopBottom(true, false, 0, CoD.textSize[CoD.QuestItemDisplay.FontName])
		Widget:setFont(CoD.fonts[CoD.QuestItemDisplay.FontName])
		Widget:setAlignment(LUI.Alignment.Left)
		Widget:addElement(Widget)
		Widget:setText(Engine.Localize("ZMUI_QUEST_ITEM"))
		f2_arg0.persQuestTitle = Widget
	end
	local Widget = LUI.UIElement.new()
	Widget:setLeftRight(true, false, 0, CoD.QuestItemDisplay.IconSize)
	Widget:setTopBottom(false, true, -CoD.QuestItemDisplay.IconSize, 0)
	Widget:addElement(Widget)
	CoD.CraftablesIcon.new(Widget, CoD.QuestItemDisplay.glowBackColor, CoD.QuestItemDisplay.glowFrontColor)
	if Widget.grunge then
		Widget.grunge:setAlpha(CoD.CraftablesIcon.GrungeAlpha)
	end
...
CoD.QuestItemDisplay.AddQuestStatusDisplay = function (f3_arg0, f3_arg1, f3_arg2, f3_arg3, f3_arg4)
	...
	if CoD.Zombie.IsDLCMap(CoD.Zombie.DLC2Maps) and f3_arg2 == true then
		...
		Widget:setTopBottom(true, false, f3_local3, f3_local3 + CoD.QuestItemDisplay.ContainerSize)
		Widget:setAlpha(1)
		f3_arg0.keyStatusContainer = Widget
		if f3_arg3 == true and f3_arg4 then
			local Widget = LUI.UIImage.new()
			Widget:setLeftRight(true, true, -f3_arg4, f3_arg4)
			Widget:setTopBottom(true, true, -f3_arg4, f3_arg4)
			Widget:setRGB(0, 0, 0)
			Widget:setAlpha(0.7)
			Widget:addElement(Widget)
		...
		Widget:setAlignment(LUI.Alignment.Left)
		Widget:addElement(Widget)
		Widget:setText(Engine.Localize("ZMUI_KEY"))
		f3_arg0.keyStatusTitle = Widget
		f3_arg0:addElement(Widget)
		local Widget = LUI.UIElement.new()
		Widget:setLeftRight(true, false, f3_local4, f3_local4 + CoD.QuestItemDisplay.IconSize)
		Widget:setTopBottom(false, true, -CoD.QuestItemDisplay.IconSize, 0)
		Widget:addElement(Widget)
		CoD.CraftablesIcon.new(Widget, CoD.QuestItemDisplay.glowBackColor, CoD.QuestItemDisplay.glowFrontColor)
	...
	local Widget = CoD.QuestItemDisplay.QuestClientFieldCount
	f3_local5 = CoD.QuestItemDisplay.IconSize * Widget + CoD.QuestItemDisplay.IconSpacing * (Widget - 1)
	local Widget = LUI.UIElement.new()
	Widget:setLeftRight(true, false, f3_local2, f3_local2 + f3_local5)
	Widget:setTopBottom(true, false, f3_local3, f3_local3 + CoD.QuestItemDisplay.ContainerSize)
	Widget:setAlpha(0)
	f3_arg0.questStatusContainer = Widget
	if f3_arg3 == true and f3_arg4 then
		local Widget = LUI.UIImage.new()
		Widget:setLeftRight(true, true, -f3_arg4, f3_arg4)
		Widget:setTopBottom(true, true, -f3_arg4, f3_arg4)
	...
	Widget:setAlignment(LUI.Alignment.Left)
	Widget:addElement(Widget)
	Widget:setText(CoD.QuestItemDisplay.QuestItemName[f3_local0])
	f3_arg0.questStatusTitle = Widget
	local f3_local9 = 0
	local f3_local10 = 5
	local f3_local11 = -2
	...
	for f3_local13 = 1, CoD.QuestItemDisplay.QuestClientFieldCount, 1 do
		local Widget = LUI.UIElement.new()
		Widget:setLeftRight(true, false, f3_local9, f3_local9 + CoD.QuestItemDisplay.IconSize)
		Widget:setTopBottom(false, true, -CoD.QuestItemDisplay.IconSize, 0)
		Widget:addElement(Widget)
		CoD.CraftablesIcon.new(Widget, CoD.QuestItemDisplay.glowBackColor, CoD.QuestItemDisplay.glowFrontColor)
		...
		Widget.crossoutImage = f3_local18
		...
		playerID:setAlpha(1)
		Widget:addElement(playerID)
		Widget.playerID = playerID
		
		f3_arg0.statusIcons[f3_local13] = Widget
