...
local SCOREBOARD_SPLITSCREEN_MAX_COLUMNS_REDUCTION = 2
local f0_local14 = 0
...
local f0_local31 = 2
local f0_local32 = 4
local ScoreboardUpdateTeamElement, ScoreboardWidgetShowGamercardFunc = nil
local f0_local36 = 460
local ScoreboardWidgetToggleMuteFunc, f0_local38, ScoreboardWidgetUpdateFunc = nil
local f0_local40 = CoD.MPZM(0, 4 * f0_local29)
local f0_local41 = f0_local40
local SCOREBOARD_MAX_ROWS = CoD.MPZM(23, 8)
local SCOREBOARD_SPLITSCREEN_MAX_ROWS = CoD.MPZM(15, 4)
local IsDLCMap2, IsDLCMap4, IsClassic = nil
...
local CreateScoreboardHeaderTitle = function (ScoreboardWidget)
	...
	if ScoreboardWidget.mode == "theater" then
		...
		StringTable[2] = Engine.Localize(UIExpression.TableLookup(nil, CoD.mapsTable, 0, Mapname, 3))
		if CoD.isZombie then
			StringTable[1] = CoD.GetZombieGameTypeName(Gametype, Mapname)
		end
		if not ScoreboardWidget.frontEndOnly and CoD.isZombie == false and Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == false then
...
function CreateScoreBoardBody(ScoreboardWidget, LocalClientIndex, UnusedArg1)
	...
	if Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == true then
		MaxColumns = MaxColumns - SCOREBOARD_SPLITSCREEN_MAX_COLUMNS_REDUCTION
	...
	if CoD.isPC and not UnusedArg1 and Dvar.tu7_scoreboardPingAsNumbers:get() == true then
		local f5_local7 = 5
	...
	ScoreboardMaxRows = SCOREBOARD_MAX_ROWS
	if Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == true then
		ScoreboardMaxRows = SCOREBOARD_SPLITSCREEN_MAX_ROWS
	...
	for ScoreboardRowIndex = 1, ScoreboardMaxRows, 1 do
...
LUI.createMenu.Scoreboard = function (LocalClientIndex)
	...
	if Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == true then
		--UPVAL3 = SCOREBOARD_SPLITSCREEN_MAX_COLUMNS_REDUCTION * f0_local7 + f0_local17 + f0_local25 + f0_local24 * 3
	end
	--UPVAL9 = f0_local40
	if (IsDLCMap2 == true or IsDLCMap4 == true) and IsClassic == true then
		--UPVAL9 = f0_local41 + 75
	end
	if Engine.IsSplitscreen() == true then
		--UPVAL9 = f0_local41 + f0_local29
	end
	...
	ScoreboardWidget:setOwner(LocalClientIndex)
	if (IsDLCMap2 == true or IsDLCMap4 == true) and IsClassic == true then
		local CraftablesHorizontalOffset = f0_local30 + f0_local6
		local f7_local2 = SCOREBOARD_DEFAULT_MAX_COLUMNS
		if Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == true then
			f7_local2 = f7_local2 - SCOREBOARD_SPLITSCREEN_MAX_COLUMNS_REDUCTION
			CraftablesHorizontalOffset = CraftablesHorizontalOffset - f0_local25 - f0_local24 * 2 - f0_local17
		end
		if IsDLCMap2 == true then
			CoD.ScoreboardCraftablesZombie.new(ScoreboardWidget, CraftablesHorizontalOffset)
		elseif IsDLCMap4 == true then
			CoD.ScoreboardCraftablesTombZombie.new(ScoreboardWidget, CraftablesHorizontalOffset)
		end
	end
...
ScoreboardWidgetRowSelectorFunc = function (ScoreboardWidget, ScoreboardRowSelected)
	...
	local f10_local0 = 0
	if Engine.IsSplitscreen() and ScoreboardWidget.forcedFullscreen ~= true then
		if not Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) then
			local f10_local1 = 0
			local f10_local2 = 9
			local f10_local3 = 13
			if f10_local2 < ScoreboardWidget.focusableRowIndex then
				if ScoreboardWidget.focusableRowIndex < f10_local3 then
					f10_local1 = ScoreboardWidget.focusableRowIndex - f10_local2
				else
		...
		ScoreboardWidget:setTopBottom(false, false, -CoD.SDSafeHeight / 2 + f0_local41 - f10_local0, CoD.SDSafeHeight / 2)
	end
...
function UpdateGameScoreboard(ScoreboardWidget)
	...
	for Key, ScoreboardTeamIndex in ipairs(ScoreboardTeams) do
		ScoreboardTeamIndex.numClients = Engine.GetMatchScoreboardClientCount(ScoreboardTeamIndex.team)
		if GreatestNumberOfClientsOnATeam < ScoreboardTeamIndex.numClients then
			GreatestNumberOfClientsOnATeam = ScoreboardTeamIndex.numClients
		end
	end
	ScoreboardMaxRows = SCOREBOARD_MAX_ROWS
	...
	if GreatestNumberOfClientsOnATeam <= math.floor(ScoreboardMaxRows / #ScoreboardTeams) then
		f18_local9 = math.max(f18_local9, GreatestNumberOfClientsOnATeam)
	end
	ClientNum = nil
	if not ScoreboardWidget.frontEndOnly then
	...
	for Key, ScoreboardTeam in ipairs(ScoreboardTeams) do
		local ScoreboardTeamNumClients = ScoreboardTeam.numClients
		if ScoreboardWidget.teamElements[TeamElementIndex] and ScoreboardTeamNumClients > 0 then
			...
			if FactionTeam then
				...
				if CoD.isZombie == true then
					...
					elseif GamemodeGroup == CoD.Zombie.GAMETYPEGROUP_ZSURVIVAL then
						...
						elseif CoD.Zombie.IsSurvivalUsingCIAModel == true then
							local FactionColorR, FactionColorG, FactionColorB = Engine.GetFactionColor("cia")
						end
					end
				end
				ScoreboardUpdateTeamElement(ScoreboardWidget.teamElements[TeamElementIndex], FactionTeam, FactionColorR, FactionColorG, FactionColorB, ScoreboardTeam, math.max(f18_local9, ScoreboardTeamNumClients), f18_local5)
				TeamElementIndex = TeamElementIndex + 1
				f18_local5 = f18_local5 + f0_local31
				for PlayerIndex = 0, ScoreboardTeamNumClients - 1, 1 do
				...
				for PlayerIndex = ScoreboardTeamNumClients + 1, f18_local9, 1 do
					local ScoreboardRow = ScoreboardWidget.rows[ScoreboardRowIndex]
	...
	if not f18_local14 and f18_local15 and not ScoreboardWidget.frontEndOnly then
		...
		if Engine.IsSplitscreen() == false or ScoreboardWidget.forcedFullscreen == true then
			...
			elseif not ScoreboardWidget.frontEndOnly then
				...
				f0_local38(ScoreboardWidget, ClientNum)
			end
		end
	end
end
...
ScoreboardUpdateTeamElement = function (TeamElement, FactionTeam, FactionColorR, FactionColorG, FactionColorB, ScoreboardTeam, f21_arg6, f21_arg7)
	local VerticalOffset = f21_arg6 * f0_local29 + (f21_arg6 + 1) * f0_local31
	...
	elseif ScoreboardTeam.team ~= CoD.TEAM_FREE then
		...
		if ScoreboardTeam.leagueTeamID and ScoreboardTeam.leagueTeamID ~= "0" then
			TeamElement.factionIcon:setupLeagueEmblem(ScoreboardTeam.leagueTeamID)
			TeamElement.factionIcon:setAlpha(1)
		else
		...
		local ScoreboardTeamName = Engine.Localize(CoD.MPZM("MPUI_", "ZMUI_") .. FactionTeam .. "_SHORT_CAPS")
		if Engine.GameModeIsMode(CoD.GAMEMODE_LEAGUE_MATCH) then
			if ScoreboardTeam.team == CoD.TEAM_ALLIES and Dvar.g_customTeamName_Allies:get() ~= "" then
				ScoreboardTeamName = UIExpression.ToUpper(nil, Dvar.g_customTeamName_Allies:get())
			elseif ScoreboardTeam.team == CoD.TEAM_AXIS and Dvar.g_customTeamName_Axis:get() ~= "" then
				ScoreboardTeamName = UIExpression.ToUpper(nil, Dvar.g_customTeamName_Axis:get())
			end
		end
...
CoD.ScoreboardRow.new = function (LocalClientIndex, ScoreboardRowIndex)
	...
	for ColumnIndex = 1, MaxColumns, 1 do
		if Engine.GetScoreBoardColumnName(LocalClientIndex, ColumnIndex - 1) ~= "" then
			if Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == false and ColumnIndex % 2 == 1 then
	...
	if Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == false then
		if CoD.isPC and true == Dvar.tu7_scoreboardPingAsNumbers:get() then
			...
			ScoreboardRowWidget.pingBars = LUI.UIImage.new()
			ScoreboardRowWidget.pingBars:setLeftRight(true, false, f23_local4, f23_local4 + f0_local24)
			ScoreboardRowWidget.pingBars:setTopBottom(false, false, -f0_local24 / 2, f0_local24 / 2)
			ScoreboardRowWidget:addElement(ScoreboardRowWidget.pingBars)
		end
		f23_local4 = f23_local4 + f0_local24
	end
	local f23_local9 = 2
...
CoD.ScoreboardRow.setClient = function (ScoreboardRow, FactionColorR, FactionColorG, FactionColorB, VerticalOffset, ScoreboardMode, ClientNum, FocusableRowIndex, PlayerIndex, ScoreboardTeam, ScoreboardFrontEndOnly)
	...
	local PlayerRank, PlayerRankIcon, PlayerPrestige, PlayerGamerTag, PlayerScoreboardIndex, PlayerScoreboardClientNum = nil
	local MaxColumns = SCOREBOARD_DEFAULT_MAX_COLUMNS
	if Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == true then
		MaxColumns = MaxColumns - SCOREBOARD_SPLITSCREEN_MAX_COLUMNS_REDUCTION
	end
